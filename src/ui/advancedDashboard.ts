...
export interface UsageStatistics {
  ...
  weekly: {
    ...
    tokens: number;
    cost: number;
    trends: Array<{ date: string; requests: number; cost: number }>;
  };
  monthly: {
    requests: number;
    tokens: number;
  ...
  costBreakdown: Record<string, { requests: number; cost: number }>;
}
...
export class AdvancedDashboardUI {
  ...
  public createDashboard(): void {
    this.panel = vscode.window.createWebviewPanel(
      'guidoAdvancedDashboard',
      '🚀 Guido Advanced Dashboard',
  ...
  private async sendUsageStatistics(period: string = 'daily'): Promise<void> {
  ...
  private async analyzeRouting(prompt: string): Promise<void> {
    try {
      // Simulate routing analysis
      const analysis: RouteVisualization = {
        prompt,
        rules: [
  ...
  private async sendOllamaModels(): Promise<void> {
  ...
  private async startOllamaModel(modelName: string): Promise<void> {
    vscode.window.showInformationMessage(`Starte Modell ${modelName}...`);
    await this.sendOllamaModels(); // Refresh to show updated status
  }

  private async stopOllamaModel(modelName: string): Promise<void> {
  ...
  private async updateRouterMode(mode: string): Promise<void> {
    // Update router configuration
    vscode.window.showInformationMessage(`Router-Modus geändert zu: ${mode}`);
  }

  private async updateBudget(budget: number): Promise<void> {
