...
Provide a consensus answer that combines the best aspects of all responses.`;
    ...
    if (request.models.length > 0) {
      ...
      if (provider) {
        const startTime = Date.now();
        try {
          const consensusResult = await provider.chatComplete(
            modelName,
          ...
          responses.push({
            modelId: "consensus",
            response: `Consensus generation failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
            metadata: {
              duration: Date.now() - startTime,
...
Rank the responses from best to worst with reasoning.`;
    ...
    if (request.models.length > 0) {
      ...
      if (provider) {
        const startTime = Date.now();
        try {
          const comparisonResult = await provider.chatComplete(
            modelName,
          ...
          responses.push({
            modelId: "comparison",
            response: `Comparison analysis failed: ${
              error instanceof Error ? error.message : String(error)
            }`,
            metadata: {
              duration: Date.now() - startTime,
