...
export class AdvancedPromptingManager {
  ...
  async generatePrompt(template: string, variables: Record<string, string>): Promise<string> {
    let prompt = template;
...
Provide your response in a structured format.`;
  ...
  private parseOptimizationResult(response: string, request: PromptOptimizationRequest): OptimizedPrompt {
  ...
  private parseAnalysisResult(response: string): {
    ...
    for (const line of lines) {
      ...
      if (clarityMatch) result.clarity = parseInt(clarityMatch[1]);
      
      const specificityMatch = lowerLine.match(/specificity.*?(\d+)/);
      if (specificityMatch) result.specificity = parseInt(specificityMatch[1]);
      
      const completenessMatch = lowerLine.match(/completeness.*?(\d+)/);
      if (completenessMatch) result.completeness = parseInt(completenessMatch[1]);
      
      const structureMatch = lowerLine.match(/structure.*?(\d+)/);
      if (structureMatch) result.structure = parseInt(structureMatch[1]);
