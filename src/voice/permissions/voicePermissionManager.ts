...
export class VoicePermissionManager {
  ...
  private dataStorage: Map<string, any> = new Map();
  private consentGiven = false;
  private lastConsentCheck?: Date;
   ...
   */
  async requestGDPRConsent(): Promise<boolean> {
...
Stimmen Sie der Verarbeitung Ihrer Sprachdaten zu?`;
    ...
    if (result === "✅ Ja, ich stimme zu") {
      ...
      vscode.window.showInformationMessage(
        "✅ Einverständnis erteilt. Sie können diese Entscheidung jederzeit in den Einstellungen widerrufen."
      );
    ...
    vscode.window.showWarningMessage(
      "❌ Voice Control kann ohne Einverständnis nicht verwendet werden. Funktionen wurden deaktiviert."
    );
  ...
  async requestMicrophonePermission(): Promise<void> {
    ...
    try {
      ...
      this.permissions.microphone = permission.state as any;
      ...
      if (this.config.microphoneAccess.showPermissionDialog) {
        const result = await vscode.window.showInformationMessage(
          "🎤 Mikrofon-Berechtigung erforderlich\n\nGuido Voice Control benötigt Zugriff auf Ihr Mikrofon für die Sprachsteuerung.",
          { modal: true },
   ...
   */
  async requestActionConfirmation(action: string, description: string): Promise<boolean> {
    if (!this.config.restrictedActions.includes(action)) {
      return true; // Not a restricted action
    }

    const result = await vscode.window.showWarningMessage(
      `🔐 Bestätigung erforderlich\n\n${description}\n\nDiese Aktion erfordert eine Bestätigung aus Sicherheitsgründen.`,
      { modal: true },
      "✅ Erlauben",
      "❌ Verweigern"
    );
  ...
  async deleteUserData(key?: string): Promise<void> {
    ...
    vscode.window.showInformationMessage(
      key ? `Daten für "${key}" gelöscht` : "Alle gespeicherten Daten gelöscht"
    );
  }
...
`;
   ...
   * Store consent data
   */
  private async storeConsentData(): Promise<void> {
    const consentData = {
      given: true,
      timestamp: new Date().toISOString(),
   ...
   */
  private logSecurityAction(action: string, allowed: boolean, description: string): void {
    const logEntry = {
      timestamp: new Date().toISOString(),
      action,
      allowed,
  ...
  private checkWorkingHours(): void {
    if (!this.isWithinWorkingHours()) {
      ...
      if (quietHours) {
        vscode.window.showInformationMessage(
          `🌙 Ruhezeiten aktiv (${this.config.workingHours.quietHours.startTime}-${this.config.workingHours.quietHours.endTime}). Lautstärke reduziert.`
        );
      } else {
        vscode.window.showWarningMessage(
          `⏰ Außerhalb der Arbeitszeiten (${this.config.workingHours.startTime}-${this.config.workingHours.endTime}). Voice Control eingeschränkt verfügbar.`
        );
      }
    }
  }
  ...
  async revokeConsent(): Promise<void> {
    const result = await vscode.window.showWarningMessage(
      "⚠️ Einverständnis widerrufen\n\nMöchten Sie Ihr Einverständnis widerrufen? Alle gespeicherten Daten werden gelöscht und Voice Control deaktiviert.",
      { modal: true },
      "Ja, widerrufen",
      "Abbrechen"
    );
    ...
    if (result === "Ja, widerrufen") {
      ...
      vscode.window.showInformationMessage(
        "✅ Einverständnis widerrufen und alle Daten gelöscht. Voice Control wurde deaktiviert."
      );
    }
  }
}
