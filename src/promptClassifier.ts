...
export type PromptClass = 
  | "boilerplate"    // Simple code generation, templates
  ...
  | "reasoning"      // Complex logic, mathematical proofs
...
export class PromptClassifier {
   ...
   */
  async classify(prompt: string, context?: {
    filePath?: string;
    language?: string;
    fileSize?: number;
  ...
  private heuristicClassification(
    prompt: string, 
    context?: { filePath?: string; language?: string; fileSize?: number }
  ): ClassificationResult {
    const lower = prompt.toLowerCase();
    const reasoning: string[] = [];
    const characteristics: ClassificationResult["characteristics"] = {
      complexity: "medium",
      creativity: "low",
      technical: "medium",
      contextSize: "medium",
    ...
    let classification: PromptClass = "general";
    let confidence = 0.5;
    ...
    let bestMatch = { class: "general" as PromptClass, score: 0 };
    ...
    for (const [className, keywords] of Object.entries(patterns)) {
      ...
      if (score > bestMatch.score) {
        bestMatch = { class: className as PromptClass, score };
  ...
  private async llmClassification(
    ...
    try {
      const result = await this.localProvider.chatComplete(
...
Classify this prompt and respond with JSON in this exact format:
{
  ...
  ): Array<{ name: string; provider: string; score: number; reason: string }> {
    const suggestions: Array<{ name: string; provider: string; score: number; reason: string }> = [];
