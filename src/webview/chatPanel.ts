...
interface InboundMessage { type: string; text?: string; attachmentUris?: string[]; modelOverride?: string; action?: string }
...
export class ChatPanel {
  ...
  private renderHtml(): string {
    ...
    body{font-family:var(--vscode-font-family);padding:0;margin:0;background:var(--vscode-editor-background);color:var(--vscode-editor-foreground);}header{display:flex;justify-content:space-between;align-items:center;padding:4px 8px;background:var(--vscode-titleBar-activeBackground);}button{background:var(--vscode-button-background);color:var(--vscode-button-foreground);border:none;padding:4px 8px;cursor:pointer;border-radius:4px;font-size:12px;}button:hover{background:var(--vscode-button-hoverBackground);}#messages{padding:8px;height:calc(100vh - 150px);overflow:auto;font-size:13px;line-height:1.4;} .msg.user{background:var(--vscode-editorHoverWidget-background);padding:6px 8px;border-radius:6px;margin:4px 0;} .msg.assistant{background:var(--vscode-editorWidget-background);padding:6px 8px;border-radius:6px;margin:4px 0;}#inputBar{display:flex;gap:6px;padding:8px;border-top:1px solid var(--vscode-panel-border);}#prompt{flex:1;resize:vertical;min-height:40px;font-family:inherit;font-size:13px;}#toolbar{display:flex;gap:4px;flex-wrap:wrap;}#attachments{font-size:11px;opacity:.8;padding:0 8px;}#modelSelect{max-width:180px;}footer{font-size:11px;opacity:.7;padding:4px 8px;border-top:1px solid var(--vscode-panel-border);} .info{font-style:italic;opacity:.8;}#voiceIndicator{font-size:11px;opacity:.8;margin-left:auto;padding:2px 6px;border-radius:4px;background:var(--vscode-badge-background);color:var(--vscode-badge-foreground);} .state-recording{background:#b30000 !important;color:#fff;} .state-processing{background:#8a6d00 !important;} .state-listening{background:#005a8a !important;}
    </style></head><body>
    <header><div id="toolbar"><select id="modelSelect" title="Modell Override"><option value="">auto</option></select><button id="btnTools" title="Tools">$(tools)</button><button id="btnMic" title="Mikrofon">$(mic)</button><button id="btnSpeaker" title="Sprachausgabe">$(unmute)</button><button id="btnAttach" title="Anhänge">$(mail)</button><button id="btnPlan" title="Plan / Agent">$(rocket)</button><button id="btnSettings" title="Settings">$(gear)</button><button id="btnMin" title="Minimieren">$(chrome-minimize)</button></div><span id="voiceIndicator" class="state-idle">idle</span></header>
