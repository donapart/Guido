...
exports.VoiceCommandProcessor = void 0;
...
class VoiceCommandProcessor {
    ...
    // System Commands
    async processSystemCommands(transcript, context) {
    ...
    // Language Commands
    async processLanguageCommands(transcript, context) {
    ...
    // Routing Commands
    async processRoutingCommands(transcript, context) {
    ...
    // VSCode Commands
    async processVSCodeCommands(transcript, context) {
        // File operations
    ...
    async processDevelopmentCommands(transcript, context) {
...
Verwende den passenden Dokumentations-Stil für die Sprache (JSDoc, DocString, etc.).`;
    ...
    // Helper methods
    async executeAICommand(prompt, context, operation) {
        try {
            ...
            const aiResult = await routingResult.provider.chatComplete(routingResult.modelName, [{ role: "user", content: prompt }], { maxTokens: 800, temperature: 0.7 });
    ...
    createSystemResponse(message, action) {
    ...
    createLanguageResponse(message, action) {
    ...
    createRoutingResponse(message, action) {
    ...
    createVSCodeResponse(message, action) {
        return {
            text: message,
            shouldSpeak: this.config.audio.ttsEnabled,
            metadata: {
    ...
    createDevResponse(message, action) {
        return {
            text: message,
            shouldSpeak: this.config.audio.ttsEnabled,
            metadata: {
    ...
    matchesAny(transcript, patterns) {
        return patterns.some(pattern => transcript.includes(pattern.toLowerCase()));
    }
    matchesCommandPattern(transcript, handler, language) {
        if (!handler.languages.includes(language)) {
            return false;
        }
        return handler.patterns.some(pattern => transcript.includes(pattern.toLowerCase()));
    }
...
exports.VoiceCommandProcessor = VoiceCommandProcessor;
//# sourceMappingURL=voiceCommandProcessor.js.map