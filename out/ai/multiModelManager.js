    ...
    async executeParallel(request) {
        const promises = request.models.map(async (model) => {
            ...
            try {
                ...
                const result = await provider.chatComplete(modelName, [
                ...
                return {
                    ...
                    metadata: {
                        ...
                        cost: 0.001 * ((result.usage?.inputTokens || 0) + (result.usage?.outputTokens || 0))
                    }
                };
            }
            catch (error) {
...
Provide a consensus answer that combines the best aspects of all responses.`;
        ...
        if (request.models.length > 0) {
            ...
            if (provider) {
                const startTime = Date.now();
                try {
                    const consensusResult = await provider.chatComplete(modelName, [
                    ...
                    responses.push({
                        ...
                        metadata: {
                            duration: Date.now() - startTime,
                            tokens: (consensusResult.usage?.inputTokens || 0) + (consensusResult.usage?.outputTokens || 0),
                            cost: 0.001 * ((consensusResult.usage?.inputTokens || 0) + (consensusResult.usage?.outputTokens || 0))
                        }
                    });
                }
                catch (error) {
...
Rank the responses from best to worst with reasoning.`;
        ...
        if (request.models.length > 0) {
            ...
            if (provider) {
                const startTime = Date.now();
                try {
                    const comparisonResult = await provider.chatComplete(modelName, [
                    ...
                    responses.push({
                        ...
                        metadata: {
                            duration: Date.now() - startTime,
                            tokens: (comparisonResult.usage?.inputTokens || 0) + (comparisonResult.usage?.outputTokens || 0),
                            cost: 0.001 * ((comparisonResult.usage?.inputTokens || 0) + (comparisonResult.usage?.outputTokens || 0))
                        }
                    });
                }
                catch (error) {
