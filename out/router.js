...
class ModelRouter {
    ...
    async route(context, options = {}) {
        const { maxFallbacks = 3, requireAvailable = true, budgetCheck = true, } = options;
        ...
        for (const scoredRule of scoredRules) {
            ...
            for (const candidate of candidates) {
                try {
                    const result = await this.validateCandidate(candidate, effectiveContext, scoredRule, { requireAvailable, budgetCheck });
                    if (result) {
                        return result;
                    }
                }
        ...
        for (const candidate of defaultCandidates) {
            try {
                const result = await this.validateCandidate(candidate, effectiveContext, { rule: null, score: 0 }, { requireAvailable, budgetCheck });
                if (result) {
     ...
     */
    scoreRule(rule, context) {
        let score = 0;
        const conditions = rule.if;
        // Keyword matching
        if (conditions.anyKeyword) {
            const matches = conditions.anyKeyword.filter(keyword => context.keywords?.some(contextKeyword => contextKeyword.toLowerCase().includes(keyword.toLowerCase())) || context.prompt.toLowerCase().includes(keyword.toLowerCase()));
            if (matches.length > 0) {
                score += matches.length * 2;
            }
        }
        if (conditions.allKeywords) {
            const allMatch = conditions.allKeywords.every(keyword => context.keywords?.some(contextKeyword => contextKeyword.toLowerCase().includes(keyword.toLowerCase())) || context.prompt.toLowerCase().includes(keyword.toLowerCase()));
            if (allMatch) {
        ...
        if (conditions.filePathMatches && context.filePath) {
            const matches = conditions.filePathMatches.filter(pattern => this.matchesPattern(context.filePath, pattern));
            if (matches.length > 0) {
     ...
     */
    async validateCandidate(candidate, context, scoredRule, options) {
