...
const historyNavigator_1 = require("./historyNavigator");
class ChatPanel {
    static current;
    panel;
    disposables = [];
    ...
    constructor(panel, extensionUri) {
        ...
        this.historyNavigator = new historyNavigator_1.HistoryNavigator();
        ...
        switch (msg.type) {
            ...
            case 'session:create':
                if (msg.name !== undefined)
                    this.createNewSession(msg.name);
                break;
            case 'session:switch':
                if (msg.sessionId)
            ...
            case 'session:rename':
                if (msg.sessionId && msg.newName)
                    this.renameSession(msg.sessionId, msg.newName);
                break;
            case 'history:search':
                if (msg.query !== undefined)
                    this.searchHistory(msg.query);
                break;
        }
    }
    addMessageToSession(content, role, meta) {
