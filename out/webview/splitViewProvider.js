...
class SplitViewProvider {
    _extensionUri;
    static viewType = 'modelRouter.splitView';
    _view;
    sessionManager;
    currentEditor;
    isListening = false;
    ...
    updateContext() {
        ...
        const contextRange = new vscode.Range(Math.max(0, selection.start.line - 10), 0, Math.min(lineCount - 1, selection.end.line + 10), editor.document.lineAt(Math.min(lineCount - 1, selection.end.line + 10)).text.length);
    ...
    async handleChatMessage(message, includeCode) {
    ...
    async replaceCodeInEditor(code, selectionData) {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
